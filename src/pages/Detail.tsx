...
export function Detail() {
  ...
  const { data: user } = useQuery<User>({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`https://sp-frontend-v11t.onrender.com/api/users/${params.login}/details`)
  ...
  const { data: repositories, isFetching } = useQuery<Repos[]>({
    ...
    queryFn: async () => {
      const response = await axios.get(`https://sp-frontend-v11t.onrender.com/api/users/${params.login}/repos`)
      return response.data
    },
    staleTime: 1000 * 60
  })
